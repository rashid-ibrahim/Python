import base64 as rtfd


def de_mike(cre):
    out = []

    # looking at the challenge file we can see the random bits added are this size
    n = 7

    for i in range(0, len(cre), n):
        out.append(cre[i])
        print('hold up')

    return ''.join(out)


def elucidate(bys):
    size = len("florSFIUEfet4565477")

    bys = bys[size-1:]
    size_literal = len('534345fdfgfgfdhty6y56yjl')
    bys = bys[size_literal-1:]

    x = bytes(bys, 'utf-8')
    x = rtfd.decodebytes(x)
    unfusc = rtfd.b64decode(bys)
    unfusc = unfusc.decode('UTF-8')
    print('sham_wow')
    return unfusc


def decrypt(sor):
    sor2 = sor[::-1]

    sorx = sor2[:20]
    sory = sor2[20:]
    sro = []

    for i in range(0, len(sorx), 3):
        sro.append(sorx[i])

    for i in range(0, len(sory), 2):
        sro.append(sory[i])

    return ''.join(sro)


def main():
    sik = "1h4ck="

    sik = de_mike(sik)
    sik = decrypt(sik)
    sik = elucidate(sik)

    print(f'Answer: {sik}')


if __name__ == '__main__':
    main()
